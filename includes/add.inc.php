...
$tempid = 0;
  ...
  if($error!="yes"){
    ...
    $quer1 = sprintf("insert into quotes (id, quote, comment, ip, 
	approved, date) values ('%d', '%s', '%s', '%s', '%d', '%s')", 
	$newid, $newquote, $comment, $ip, 1, date("D, d M Y H:i:s e"));
	$sql = $dbc->query($quer1);
...
global $dbc;
  ...
  <div width=80% align=center>
    <form action='./?add' name='add' method='POST'>
      <p>$lang[add_comment_2] <input name='comment' type='text' size='40' maxlength='127'></p>
      <p><textarea cols='80' rows='5' name='newquote' class='text'></textarea></p>
      <p>Username: <input name='username' type='text' size='40' maxlength='127'></p>
      <p>Password: <input name='password' type='text' size='40' maxlength='127'></p>
